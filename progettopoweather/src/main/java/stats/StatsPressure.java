...
public class StatsPressure extends Stats {
		...
		public JSONArray statsHumidityDay (JSONObject vettore) {
			...
			for(Object ob1:array1) {
				...
				if((double)ob2.get("pressure")>max )
						max=(double)ob2.get("pressure");
				if((double)ob2.get("pressure")<min)
					min=(double)ob2.get("pressure");
			    c++;
